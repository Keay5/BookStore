...
Necessary helm charts are created under `etc/k8s/bookstore` folder. Based on your back-office application UI, remove the other UI charts. Ex, if you are using **angular** UI, remove **web**, **blazor** and **blazor-server** charts.

## Generate Signing-Certificate for AuthServer:
Navigate to `auth-server/Acme/src` folder and run:
```ps
dotnet dev-certs https -v -ep ./authserver.pfx -p 2D7AA457-5D33-48D6-936F-C48E5EF468ED
``` 
to generate pfx file for signing tokens by AuthServer. Edit `Acme.BookStore.AuthServer.csproj` file and uncomment item group section related to authserver.pfx:
```cs
<!-- <ItemGroup>
...
````powershell
127.0.0.1 bookstore-st-web
127.0.0.1 bookstore-st-public-web
127.0.0.1 bookstore-st-authserver
127.0.0.1 bookstore-st-identity
127.0.0.1 bookstore-st-administration
127.0.0.1 bookstore-st-product
127.0.0.1 bookstore-st-saas
127.0.0.1 bookstore-st-gateway-web
127.0.0.1 bookstore-st-gateway-web-public
````
...
* Run `deploy-staging.ps1` in the `helm-chart` directory. It is deployed with the `bookstore` namespace.

* *You may wait ~30 seconds on first run for preparing the database*.

* Browse https://bookstore-st-public-web for public and
   https://bookstore-st-web for the back-office application
